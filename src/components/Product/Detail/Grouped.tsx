  ...
  const handleAddToCart = () => {
    if (
      !cartState.cartArray.find(
        (item) =>
          item.id === productMain.id ||
          item.id === data[Number(productId)].id ||
          item.id === data[Number(productId) + 1].id ||
          item.id === data[Number(productId) + 2].id,
      )
    ) {
      ...
      updateCart(
        data[Number(productId)].id,
        quantity.id,
        activeSize,
        activeColor,
      );
      updateCart(
        data[Number(productId) + 1].id,
        quantity.id,
        activeSize,
        activeColor,
      );
      updateCart(
        data[Number(productId) + 2].id,
        quantity.id,
        activeSize,
        activeColor,
      );
                      ...
                      <div className="left flex items-center gap-5">
                        ...
                        <div className="infor">
                          ...
                          <div className="select-block relative w-fit mt-2">
                            <select className="text-button py-2 pl-3 pr-8 rounded-lg bg-white border border-line">
